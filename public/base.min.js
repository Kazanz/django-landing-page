...
eventApp.config(['$httpProvider',function($httpProvider){ $httpProvider.defaults.xsrfCookieName='csrftoken';$httpProvider.defaults.xsrfHeaderName='X-CSRFToken'; $httpProvider.defaults.headers.post['Content-Type']='application/x-www-form-urlencoded';$httpProvider.defaults.transformRequest.unshift(function(data,_){return serializeData(data);});}]);eventApp.config(function($interpolateProvider){$interpolateProvider.startSymbol('{$');return $interpolateProvider.endSymbol('$}');});eventApp.controller('eventCtrl',function($scope){$scope.occurance=OCCURANCE;});eventApp.controller('addMemberCtrl',function($scope,$timeout){$scope.selected_members={};$scope.invited=0;$scope.updateSelected=function(id){updateCount();};$scope.initMember=function(pk){$scope.selected_members[pk]=(EVENT_MEMBERS.indexOf(pk)!=-1)?true:false;updateCount();};function updateCount(){var count=0;for(key in $scope.selected_members){if($scope.selected_members[key])count++;}
$scope.invited=count;}});eventApp.directive('dayPicker',function(){return{restrict:'E',templateUrl:"day_picker.html",scope:{'occurrance':'='},controller:function($scope){$scope.day=DAYS;$scope.time=TIME;$scope.dayClick=function(day_num){$scope.day=day_num;};}}});